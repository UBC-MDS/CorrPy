...
def test_type():
    with pytest.raise(TypeError):
        ...
        corrPy.cov_max(np.array([mix_type_x,mix_type_y])) # fail if wrong type
        corrPy.cov_max(pos_neg_x) # fail if it is 1D array

# Test the output shape
def test_length():
    assert corrPy.cov_max(np.array([single_x, single_y])) == None # two single value return none
